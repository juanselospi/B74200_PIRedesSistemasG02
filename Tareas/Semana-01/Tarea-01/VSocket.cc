...
int VSocket::TryToConnect( const char * hostip, int port ) {
   ...
   if(this->IPv6 == false) {
      ...
      st = ::connect(this->idSocket, reinterpret_cast<sockaddr*>(&host4), sizeof(host4)); // esto intenta abrir la conexion TCP
      ...
      st = ::connect(idSocket, reinterpret_cast<sockaddr*>(&host6), sizeof(host6));
...
int VSocket::TryToConnect( const char *host, const char *service ) {
   ...
   struct addrinfo hints, *listDir, *listPointer;

   memset(&hints, 0, sizeof(struct addrinfo)); // limpio memoria de estructura de hints

   hints.ai_family = AF_UNSPEC; // permite IPv4 o IPv6

   hints.ai_socktype = SOCK_STREAM; // socket tipo TCP

   hints.ai_protocol = 0; // protocolo cualquiera

   st = getaddrinfo(host, service, &hints, &listDir); // resolver host y servicio

   if(st != 0) {
      throw std::runtime_error( "VSocket::TryToConnect: getaddrinfo fallo" );
   }

   // recorro las direcciones posibles en la lista
   for(listPointer = listDir; listPointer != nullptr; listPointer = listPointer->ai_next) {

      st = connect(this->idSocket, listPointer->ai_addr, listPointer->ai_addrlen);

      if(st == 0) {
         break; // luego de probar conectarme a todas hago break si la conexion fue exitosa
      }

   }
   
   freeaddrinfo(listDir);

   // si ninguna direccion funciona
   if(listPointer == nullptr) {
      throw std::runtime_error("VSocket::TryToConnect: conexion fallida");
   }
